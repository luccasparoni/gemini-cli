...
- **Returning Rich Content:** Tools are not limited to returning simple text. The `llmContent` can be a `PartListUnion`, which is an array that can contain a mix of `Part` objects (for images, audio, etc.) and `string`s. This allows a single tool execution to return multiple pieces of rich content. The `CoreToolScheduler` is responsible for correctly processing this array, packaging all text content into a single `FunctionResponse`, and sending any binary data (like images) as separate top-level `Part`s for the model to understand.
